...
public class MBuild extends Manager{
	...
	private int realBuild(AGUnit unit, TBuild task){
		...
		task.setRepeat(0);
		unit.setTask(new TBuild(ai, ai.getManagers().get(MBuild.class), null, pos, 0, 0, task));
	...
	private void BuildUnit(UnitDef unit){
		ai.msg(unit.getName());
		AGUnit builder=ai.getUnits().getUnit(ai.getUnits().getUnitDef("armcom")); //FIXME search for commander (should search the "next" unit in tree to build)
		if (builder==null) //no commander found, search builder
			ai.msg("Commander is dead, here is missing some code...");
		IBuildTreeUnit tmp=ai.getInfos().getAGB().searchNode(builder.getUnit().getDef(),unit);
		if (tmp!=null){
			while(tmp.getUnit()!=builder.getUnit().getDef()){
				TBuild cur=new TBuild(ai, ai.getManagers().get(MBuild.class), tmp.getUnit(),null, AGAI.searchDistance, AGAI.minDistance, null);
				cur.setSolved();
	...
	public void solve(Task task) {
		...
		List <AGUnit> builder=ai.getInfos().getAGB().getBuilder(t.getUnitDef());
