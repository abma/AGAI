...
public class TSecureMove extends Task{
	...
	public TSecureMove(AGAI ai, Manager manager, Task taskWhenReached, ISector destination) {
		super(ai, manager);
	...
	public void unitCommandFinished(AGUnit unit){
		if (ai.getInfos().getAGM().isPosInSec(unit.getPos(),destination)){
			...
			ai.msg("Sneak moving");
			if (path==null){
				ISector cursec=ai.getInfos().getAGM().getSector(unit.getPos());
				path=ai.getInfos().getAGM().getSecurePath(cursec, destination);
			}
			if (path.size()>0)
